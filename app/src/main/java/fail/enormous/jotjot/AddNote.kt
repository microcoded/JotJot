...
class AddNote : AppCompatActivity() {
    ...
    fun addNote(view: View) {
        ...
        addNoteToFile(titleText, contentText)
        "Notes".goto()
    }

    private fun addNoteToFile(title: String, content: String) {
        val saveTime = System.currentTimeMillis().toString()
        val jotList = mutableListOf(
            Jot("note", saveTime, title, content, 0)
        )
    ...
    private fun String.goto() {
        if (this == "Notes") {
            startActivity(Intent(this@AddNote, Notes::class.java), ActivityOptions.makeSceneTransitionAnimation(this@AddNote).toBundle())
